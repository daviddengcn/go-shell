...
// Returns the value itself not the pointer.
func (mch *machine) evalExpr(ns NameSpace, expr ast.Expr) (reflect.Value, error) {
	...
	case *ast.Ident:
		...
		if v == noValue {
			return noValue, fmt.Errorf("Unknown Ident %v", expr.Name)
		}
		return reflect.Indirect(v), nil

	case *ast.CallExpr:
		fn, err := mch.evalExpr(ns, expr.Fun)
		if err != nil {
			return noValue, err
		}

		if fn.Kind() != reflect.Func {
			return noValue, fmt.Errorf("cannot call non-function (type %s)", fn.Type())
		}

		args := make([]reflect.Value, len(expr.Args))
		for i, arg := range expr.Args {
			argV, err := mch.evalExpr(ns, arg)
			if err != nil {
				return noValue, err
			}
			args[i] = argV
	...
	case *ast.SelectorExpr:
		...
		case *ast.Ident:
			return mch.findSelected(ns, x.Name, expr.Sel.Name)
		default:
